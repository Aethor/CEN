...
def test(
    ...
    if mode == "test":
        ...
        print(
            "Load Model name: {}. Using best epoch : {}".format(
    ...
    for time_idx, test_snap in enumerate(test_list):
        ...
        final_score, final_r_score = model.predict(
            history_glist, test_triples_input, use_cuda
        )
...
def continual_test(
    ...
    if not os.path.exists(model_name):
        ...
        if mode == "test":
            checkpoint = torch.load(
                "{}-flr{}-norm{}-{}".format(
                    model_name, args.ft_lr, args.norm_weight, start_idx - 1
                ),
        ...
        print(
            "Load pretrain model: {}. Using best epoch : {}".format(
                model_name, checkpoint["epoch"]
            )
        )  # use best stat checkpoint
    ...
    valid_input_list = [
        snap for snap in history_list[-args.test_history_len - 2 : -2]
    ]  # history for ft training (, tc-2)
    valid_snap = history_list[-2]  # snapshot for ft training at snapshot at tc-2
    ft_input_list = [
        snap for snap in history_list[-args.test_history_len - 1 : -1]
    ]  # history for ft validation (,tc-1)
    ft_snap = history_list[-1]  # snapshot for ft validation snapshot at tc-1
    test_input_list = [
        snap for snap in history_list[-args.test_history_len :]
    ]  # history for testing (, tc)
    # ft_output_list = history_list[-args.test_history_len:]

    eval_paper_authors_logging_dict = {}  ## ADDED  eval_paper_authors

    # starting continual learning
    for time_idx, test_snap in enumerate(
        data_list
    ):  # comment eval_paper_authors: loop through e.g. test set
        tc = start_idx + time_idx
        print("-----------------------{}-----------------------".format(tc))
        # step 1: get the history graphs for ft training : ft_input_list -> ft_snapshot
        ft_history_glist = [
            build_sub_graph(num_nodes, num_rels, g, use_cuda, args.gpu)
            for g in ft_input_list
        ]
        ft_tensor = (
            torch.LongTensor(ft_snap).cuda() if use_cuda else torch.LongTensor(ft_snap)
        )
        ft_tensor = ft_tensor.to(args.gpu)
        ft_output_tensor = (
            [torch.from_numpy(_).long().cuda() for _ in test_input_list]
            if use_cuda
            else [torch.from_numpy(_).long() for _ in test_input_list]
        )

        # step 2: get the history graphs for ft validation : valid_input_list -> valid_snap
        valid_history_glist = [
            build_sub_graph(num_nodes, num_rels, g, use_cuda, args.gpu)
            for g in valid_input_list
        ]
        valid_tensor = (
            torch.LongTensor(valid_snap).cuda()
            if use_cuda
            else torch.LongTensor(valid_snap)
        )
        ...
        final_score, final_r_score = model.predict(
            valid_history_glist, valid_tensor, use_cuda
        )
        mrr_filter_valid_snap_r, mrr_valid_snap_r, rank_raw_r, rank_filter_r = (
            utils.get_total_rank(
                valid_tensor,
                final_r_score,
                all_ans_r_list[tc - 2],
                eval_bz=1000,
        ...
        mrr_filter_valid_snap, mrr_valid_snap, rank_raw, rank_filter = (
            utils.get_total_rank(
                valid_tensor,
                final_score,
                all_ans_list[tc - 2],
        ...
        final_score, final_r_score = model_initial.predict(
            test_history_glist, test_tensor, use_cuda
        )
        mrr_filter_test_snap_r, mrr_test_snap_r, rank_raw_r, rank_filter_r = (
            utils.get_total_rank(
                test_tensor,
                final_r_score,
                all_ans_r_list[tc],
        ...
        mrr_filter_test_snap, mrr_test_snap, rank_raw, rank_filter = (
            utils.get_total_rank(
        ...
        while ft_epoch < args.ft_epochs:
            ...
            if mrr_filter_valid_snap >= best_mrr:
                ...
                torch.save(
                    ...
                    "{}-flr{}-norm{}-{}".format(
                        model_name, args.ft_lr, args.norm_weight, tc
                    ),
                )
                # update_flag = True
                ...
                if not os.path.exists(
                    ...
                    print(
                        mrr_filter_valid_snap,
                        best_mrr,
                        "copy model at {}".format(tc - 1),
                    )
                    if mode == "valid" and time_idx == 0:
                        ...
                        checkpoint = torch.load(
                            "{}-flr{}-norm{}-{}".format(
                                model_name, args.ft_lr, args.norm_weight, tc - 1
                            ),
                            map_location=torch.device(args.gpu),
                        )
                    model.load_state_dict(checkpoint["state_dict"])
        ...
        checkpoint = torch.load(
            "{}-flr{}-norm{}-{}".format(model_name, args.ft_lr, args.norm_weight, tc),
            map_location=torch.device(args.gpu),
        )
        model.load_state_dict(checkpoint["state_dict"])
        model.eval()
        ...
        # step 4: evaluation
        mrr_filter_snap_r, mrr_snap_r, rank_raw_r, rank_filter_r = utils.get_total_rank(
            test_tensor, final_r_score, all_ans_r_list[tc], eval_bz=1000, rel_predict=1
        )
        mrr_filter_snap, mrr_snap, rank_raw, rank_filter = utils.get_total_rank(
            test_tensor, final_score, all_ans_list[tc], eval_bz=1000, rel_predict=0
        )
...
def run_experiment(args, n_hidden=None, n_layers=None, dropout=None, n_bases=None):
    ...
    all_ans_list = utils.load_all_answers_for_time_filter(
        total_data, num_rels, num_nodes, False
    )
    all_ans_list_r = utils.load_all_answers_for_time_filter(
        total_data, num_rels, num_nodes, True
    )
    ...
    test_model_name = "{}-{}-ly{}-dilate{}-his{}-dp{}-gpu{}".format(
        args.encoder,
        args.decoder,
        args.n_layers,
        args.dilate_len,
    ...
    if args.test == 1:  # normal test on validation set  Note that mode=test
        ...
        print(
            "----------------------------------------start pre training model with history length {}----------------------------------------\n".format(
                args.start_history_len
            )
        )
        model_name = "{}-{}-ly{}-dilate{}-his{}-dp{}-gpu{}".format(
            args.encoder,
            args.decoder,
            args.n_layers,
            args.dilate_len,
        ...
        for epoch in range(args.n_epochs):
            ...
            for train_sample_num in tqdm(idx):
                optimizer.zero_grad()
                if train_sample_num == 0 or train_sample_num == 1:
                    continue
                if train_sample_num - args.start_history_len < 0:
                    ...
                    input_list = train_list[
                        train_sample_num - args.start_history_len : train_sample_num
                    ]
                    output = train_list[
                        train_sample_num - args.start_history_len + 1 : train_sample_num
                        + 1
                    ]
                ...
                output = (
                    [torch.from_numpy(_).long().cuda() for _ in output]
                    if use_cuda
            ...
            print(
                "His {:04d}, Epoch {:04d} | Ave Loss: {:.4f} | Best MRR {:.4f} | Model {} ".format(
                    args.start_history_len, epoch, np.mean(losses), best_mrr, model_name
                )
            )
        ...
        init_checkpoint = torch.load(
            init_state_file, map_location=torch.device(args.gpu)
        )
        print(
            "Load Previous Model name: {}. Using best epoch : {}".format(
                init_state_file, init_checkpoint["epoch"]
            )
        )  # use best stat checkpoint
        print(
            "\n"
            + "-" * 10
            + "Load model with history length {}".format(args.start_history_len)
            + "-" * 10
            + "\n"
        )
        ...
        mrr_raw, mrr_filter, mrr_raw_r, mrr_filter_r = test(
            model,  # modified eval paper authors: do not feed test_list
            args.start_history_len,
            train_list,
            valid_list,
            num_rels,
        ...
        ks_idx = 0
        for history_len in range(
            args.start_history_len + 1, args.train_history_len + 1, 1
        ):
            # current model
            print("best mrr list :", best_mrr_list)
            # lr = 0.1*args.lr - 0.002*args.lr*ks_idx
            optimizer = torch.optim.Adam(
                model.parameters(), lr=0.1 * args.lr, weight_decay=0.00001
            )
            model_name = "{}-{}-ly{}-dilate{}-his{}-dp{}-gpu{}".format(
                args.encoder,
                args.decoder,
            ...
            prev_model_name = "{}-{}-ly{}-dilate{}-his{}-dp{}-gpu{}".format(
                args.encoder,
                args.decoder,
                args.n_layers,
                args.dilate_len,
            ...
            checkpoint = torch.load(
                prev_state_file, map_location=torch.device(args.gpu)
            )
            ...
            print(
                "\n"
                + "-" * 10
                + "start knowledge distillation for history length at "
                + str(history_len)
                + "-" * 10
                + "\n"
            )
            ...
            for epoch in range(args.n_epochs):
                ...
                for train_sample_num in idx:
                    ...
                    if train_sample_num - history_len < 0:
                        ...
                        input_list = train_list[
                            train_sample_num - history_len : train_sample_num
                        ]
                        output = train_list[
                            train_sample_num - history_len + 1 : train_sample_num + 1
                        ]
                ...
                print(
                    "His {:04d}, Epoch {:04d} | Ave Loss: {:.4f} |Best MRR {:.4f} | Model {} ".format(
                        history_len, epoch, np.mean(losses), best_mrr, model_name
                    )
                )
        ...
        print(
            args.dataset,
            "done with knowledge distillation. stopped with best test history len: ",
            test_history_len,
        )  # eval paper authors
        mrr_raw, mrr_filter, mrr_raw_r, mrr_filter_r = test(
            model,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--test", type=int, default=0, help="1: formal test 2: continual test"
    )
    ...
    parser.add_argument(
        "--weight", type=float, default=1, help="weight of static constraint"
    )
    parser.add_argument(
        "--task-weight", type=float, default=1, help="weight of entity prediction task"
    )
    parser.add_argument(
        "--kl-weight", type=float, default=0.7, help="weight of entity prediction task"
    )

    parser.add_argument(
        "--encoder", type=str, default="uvrgcn", help="method of encoder"
    )
    ...
    parser.add_argument(
        "--n-basis", type=int, default=100, help="number of basis vector for compgcn"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--ft_epochs", type=int, default=30, help="number of minimum fine-tuning epoch"
    )
    parser.add_argument("--ft_lr", type=float, default=0.001, help="learning rate")
    parser.add_argument("--norm_weight", type=float, default=0.1, help="learning rate")
    parser.add_argument(
    ...
    parser.add_argument(
        "--decoder", type=str, default="convtranse", help="method of decoder"
    )
    parser.add_argument(
        "--input-dropout", type=float, default=0.2, help="input dropout for decoder "
    )
    parser.add_argument(
        "--hidden-dropout", type=float, default=0.2, help="hidden dropout for decoder"
    )
    parser.add_argument(
        "--feat-dropout", type=float, default=0.2, help="feat dropout for decoder"
    )
    ...
    parser.add_argument(
        "--start-history-len", type=int, default=1, help="start history length"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        "--tune",
        type=str,
        default="n_hidden,n_layers,dropout,n_bases",
        help="stat to use",
    )
    parser.add_argument(
